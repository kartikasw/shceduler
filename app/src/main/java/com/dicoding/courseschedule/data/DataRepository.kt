...
class DataRepository(private val dao: CourseDao) {
    ...
    fun getNearestSchedule(queryType: QueryType) : LiveData<Course?> {
        return dao.getNearestSchedule(nearestQuery(queryType)) ?: throw NotImplementedError("needs implementation")
    }
    ...
    fun getAllCourse(sortType: SortType): LiveData<PagedList<Course>> {
        return LivePagedListBuilder(
            ...
            PagedList.Config.Builder()
                ...
                .build())
            .build() ?: throw NotImplementedError("needs implementation")
    }

    fun getCourse(id: Int) : LiveData<Course> {
        return dao.getCourse(id) ?: throw NotImplementedError("needs implementation")
    }

    fun getTodaySchedule() : List<Course> {
        return dao.getTodaySchedule(
            Calendar.getInstance()
                .get(Calendar.DAY_OF_WEEK)) ?: throw NotImplementedError("needs implementation")
    }
